...
public class TimerClass : ScriptableObject
{
    ...
    public void setAvgWindow(int winSize)
    {
        windowSize = winSize;
    }

    public void calcAvg(float sample, ref float average, ref float min, ref float max)
    {
        average = average * (windowSize - 1) / windowSize + sample / windowSize;
